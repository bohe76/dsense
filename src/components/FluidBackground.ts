...
function createDoubleFBO(width: number, height: number, type: THREE.TextureDataType) {
...
export class FluidBackground {
    ...
    private curlMaterial: THREE.ShaderMaterial | null = null; // Optional
    ...
    private splatStack: Array<{ x: number, y: number, dx: number, dy: number, color: THREE.Vector3 }> = [];
    ...
    constructor(containerInfo: HTMLElement) {
        ...
        if (!this.renderer.capabilities.isWebGL2 && !this.renderer.extensions.get('OES_texture_half_float')) {
        ...
        this.simResX = 128;
        this.simResY = Math.floor(128 / aspect); // rough logic
        ...
        this.advectionMaterial = new THREE.ShaderMaterial({
            uniforms: {
                ...
                uSource: { value: null },
                uDt: { value: 0.016 },
                uTexelSize: { value: new THREE.Vector2(1 / this.simResX, 1 / this.simResY) },
        ...
        for (let i = 0; i < 5; i++) {
            this.splatStack.push({
                ...
                color: new THREE.Vector3(Math.random(), Math.random(), Math.random()).multiplyScalar(5.0) // Bright colors
            });
        }
    }
    ...
    step(dt: number) {
        ...
        for (const splat of this.splatStack) {
            ...
            this.splatMaterial.uniforms.uRadius.value = 0.005;
        ...
        for (let i = 0; i < 20; i++) { // Iterations count affects quality
